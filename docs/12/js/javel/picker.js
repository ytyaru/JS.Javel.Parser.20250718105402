...
//                console.log('同一TB内　SWAP!!!!!!!')
            ...
            */
                ...
                for (let i=0; i<I; i++) {
                    ...
                    if (1===childNodes[i].nodeType) {
                        ...
                        //return Number(inlineDa.bis) + (isEnd ? Number(inlineDa.len) : 0)
                        /*
    ...
    #isInlineElDataAttr(node) {// テキストブロックからみてInline要素であればそのdatasetを返す
        if ('RT RP'.split(' ').some(n=>n===node.tagName)) {//ルビ文字
            return this.#isInlineElDataAttr(node.parentElement);
            /*
            if ('RUBY'===node.parentElement.parentElement.tagName) {
                if ('bis len'.split(' ').every(n=>n in node.parentElement.dataset)) {
                    return node.parentElement.dataset;
//                    return Number(node.parentElement.parentElement.dataset.bis) + (isEnd ? Number(node.parentElement.parentElement.dataset.len) : 0)
                }
            }
            */
            ...
            if ('bis len'.split(' ').every(n=>n in node.dataset)) {
                return node.dataset;
            }
        }
        /*
        else if ('RUBY'===node.tagName) {//親文字
            if ('bis len'.split(' ').every(n=>n in node.dataset)) {
                //return Number(node.parentElement.dataset.bis) + (isEnd ? Number(node.parentElement.dataset.len) : 0)
                return node.dataset;
            }
        }
        else if ('EM'===node.tagName) {
            if ('bis len'.split(' ').every(n=>n in node.dataset)) {
                //return Number(node.parentElement.dataset.bis) + (isEnd ? Number(node.parentElement.dataset.len) : 0)
                return node.dataset;
            }

...
//    splitter.target = Dom.q(`[name="book"]`);
    ...
    Dom.q(`[name="view"]`).addEventListener('click', async(e)=>{
        ...
        for await (let page of splitter.generateAsync()) {
            console.log('ページ数:',page.dataset.page, page)
            //if ('1'==page.dataset.page) {Dom.q(`.page:not(.dummy)`).classList.add('show');console.log(Dom.q(`.page:not(.dummy)`));}
            if ('1'==page.dataset.page) {Dom.q(`.page`).classList.add('show');console.log(Dom.q(`.page`));}
//            if ('1'==page.dataset.page) {page.classList.add('show');console.log(Dom.q(`.page`));}

class PageSplitter {
    ...
    #makeCover() {
        this._.dummy.el.append(
            Dom.tags.h1({'data-name':'title'}, this._.jp.meta.el.title[0]),
            Dom.tags.p({'data-name':'author.name'}, this._.jp.meta.el.author.name),
        ...
        for (let i=0; i<graphemes.length; i++) {
            ...
            if (this._.dummy.without) {
                ...
                if (0===p.textContent.length) {// 一文字も入らない（si=-1のはず）
                    ...
                    this.#splitGraphemes(graphemes.slice(i), bi, si); // si=-1のはず
                } else {// 一文字以上ある
                    ...
                    console.log(lastNode.textContent);
                    this.#makePage(null, bi, SI);
                    this.#splitGraphemes(graphemes.slice(i), bi, SI+1);
                }
                return this._.dummy.el.querySelector(`p:last-child`);
            }
        }
